...
# -- init
API_THREAD = 'http://a.4cdn.org/%s/thread/%d.json'  # (board, thread_no)
API_INDEX = 'http://a.4cdn.org/%s/%d.json'  # (board, range(1, PAGELIMIT-1))
API_CATALOG = 'http://a.4cdn.org/%s/catalog.json'
CATALOG_ORDER_TYPES = ['absdate', 'date', 'alt', 'r']
THREADURL = 'https://boards.4chan.org/%s/thread/%d'  # (board, thread_no)
BOARDCACHE = dict()
THREADCACHE = dict()
HTTP_HEADERS = {
    ...
    if len(BOARDCACHE[board]['threads']) == 0:
        ...
        if res.status >= 200 and res.status < 300:
            try:
                ...
                json_list = ()
            for i in json_list:  # [{'page': 1, 'threads:[{},{},{},...]},...]
                for thread in i['threads']:
                    thread['ctime'] = float(thread['time'])
                    if 'last_replies' in thread:
                        thread['mtime'] = float(thread['last_replies'][-1]['time'])
                        del thread['last_replies']
                    else:
                        thread['mtime'] = thread['ctime']
                    BOARDCACHE[board]['threads'].append(thread)
        else:
...
def tell_4chan_thread(phenny, cmd_in):
    ...
    if cmd_in.admin:
        pass
    elif (cooldown is None) or (searchconfig is None):
        return
    elif cooldown < 0:
